...
class GameInstance(ABC):
    @abstractmethod
    def __getitem__(self, item: int) -> Piece | None:
        """
        Returns a piece located on the square with id given by item.
        Implementations should return None if item < 0 or item >= 0
        :param item: id of a square
        :return: Piece if piece is present, None if square is None.
        """
        pass
    ...
    def get_side(self) -> Side | None:
        """
        1. If state was set, it returns side to move
        2. If game was finished, it returns side of a loser
        3. If state was not set, it returns None
        :return:
        """
        pass
    ...
    def is_move_legal(self, move: tuple[int, int]) -> bool:
        """
        Returns true if given move is legal in given position
        :param move: a tuple in form (from, to), where from and to are ids of squares
    ...
    def is_game_finish(self) -> bool:
        """
        :return: True if game was finished, False if game is ongoing
        """
        pass
    ...
    def get_winner(self) -> Side | None:
        """
        :return: Side of a winner, None if game is ongoing
        """
        pass
    ...
    def set_game_state(self, new_game_state: GameState):
        ...
        :param new_game_state: A new game state to be set on the board
        :return:
        """
        pass
    ...
    def get_board(self) -> list[Piece | None]:
        """
        :return: A board state in form of the list of pieces
        """
    ...
    def move_gen(self, side: Side) -> list[tuple[int, int]]:
        """
        Returns list of moves available in given position
        :param side:
        :return:
        """
        pass
    ...
    def get_view(self, side: Side | None) -> GameStateView:
        ...
        """
        result = [self.__generate_token(self[i], side) for i in range(100)]
        return GameStateView(result)


class PurePythonGameInstance(GameInstance):
    """
        The main
    """
    def __init__(self):
    ...
    def is_game_finish(self):
        for side in Side:
            if self._mobile_tracker[side] == 0 or self._flag_captured[side]:
                return True
        if len(self.move_gen(self._side_to_move)) == 0:
            return True
        return False
